			...
			OPCODE 48: // Emulator-specific 0F xx opcodes
				switch ((char)i_data0)
				{
					...
					OPCODE_CHAIN 3: // DISK_WRITE
                        {
                            ssize_t byteRes = 0;
                            auto res = ~lseek(disk[regs8[REG_DL]], CAST(unsigned)regs16[REG_BP] << 9, 0);
                            if((char)i_data0 == 3)
                            {
                                byteRes = write(disk[regs8[REG_DL]], mem + SEGREG(REG_ES, REG_BX,), regs16[REG_AX]);
                            }
                            else
                            {
                                byteRes = read(disk[regs8[REG_DL]], mem + SEGREG(REG_ES, REG_BX,), regs16[REG_AX]);
                            }
                            regs8[REG_AL] = byteRes;
                        }
				}
		}
